<?php 
  ...
  mysql_set_charset('utf8');
    if($_POST['enviar'] == "ALTA"){
      ...
      mysql_query("INSERT INTO cliente VALUES ('$codigo','$precioCompra','$descripcion','$precioVenta')",$conexion); 
    } 
    if($_POST['enviar'] == "BAJA"){
      $codigo = $_POST['dni'];
      $consulta = "DELETE FROM cliente WHERE dni = '$codigo'";
      mysql_query("DELETE FROM cliente WHERE dni = '$codigo'",$conexion); 
      echo $consulta;
    } 
  ...
  <article>
    ...
    <table class="highlight centered">
      ...
      <?php 
              ...
              $codigo = $_POST['dni'];
              $registro = mysql_query("SELECT * FROM cliente WHERE dni = '$codigo'", $conexion);
              $codigo = mysql_result($registro,0,'dni');
              $precioCompra = mysql_result($registro,0,'nombre');
              $descripcion = mysql_result($registro, 0,'direccion');
              $precioVenta = mysql_result($registro,0,'telefono');
      ?>
